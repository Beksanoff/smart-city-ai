...
### Визуализация данных
- 🗺️ **Интерактивная карта** — линейная карта трафика 24 дорог Алматы (Deck.gl PathLayer + MapLibre GL) с цветовой индикацией загруженности и маркерами инцидентов
...
- 🔌 **Реальные данные** — OpenWeatherMap (погода + AQI), TomTom (трафик + инциденты), Groq (LLM-прогнозы)
- 🔄 **Кэширование** — трафик кэшируется на 3 мин для экономии API-лимитов
- 📜 **Историческая аналитика** — API эндпоинты для запроса истории погоды и трафика
...
GET /api/v1/traffic
```
Возвращает данные о трафике: 24 дорожных сегмента с координатами и уровнем загруженности, а также инциденты (ДТП, ремонт, полиция).

#### Weather History
```http
GET /api/v1/history/weather?hours=24
```
Возвращает историю погоды за последние N часов (max 720 = 30 дней).

#### Traffic History
```http
GET /api/v1/history/traffic?hours=24
```
Возвращает историю трафика за последние N часов.

### ML Service Endpoints

#### Prediction (через Backend)
```http
POST /api/v1/predict
...
```
Возвращает статистику по историческим данным (1096 записей, 2023–2025).
